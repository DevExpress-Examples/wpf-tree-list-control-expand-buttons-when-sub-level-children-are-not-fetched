...
namespace HasChildNodes {
    ...
    public class ViewModel {
        ...
        void InitBetaronProjectData(ProjectObject betaronProject) {
            ...
            Task13.Tasks.Add(new Task() { Name = "Design of a web pages", Executor = "Gasper Hartsell", State = "Not started", HasChildNodes = false });
            Task13.Tasks.Add(new Task() { Name = "Pages layout", Executor = "Shirish Huminski", State = "Not started", HasChildNodes = false });
        ...
        void InitStantoneProjectData(ProjectObject stantoneProject) {
            ...
            Task23.Tasks.Add(new Task() { Name = "Design of a web pages", Executor = "Dominic Minden", State = "In progress", HasChildNodes = false });
            Task23.Tasks.Add(new Task() { Name = "Pages layout", Executor = "Pinkerton Trezise", State = "In progress", HasChildNodes = false });

            Task Task24 = new Task() { Name = "Development", Executor = "Lauren Partain", Tasks = new ObservableCollection<Task>(), HasChildNodes = true };
            Task24.Tasks.Add(new Task() { Name = "Design", Executor = "Delilah Beamer", State = "In progress", HasChildNodes = false });
            Task24.Tasks.Add(new Task() { Name = "Coding", Executor = "Dunaway Dupriest", State = "Not started", HasChildNodes = false });
